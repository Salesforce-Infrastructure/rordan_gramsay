require_relative '../../lib/rordan_gramsay/lint/rubocop'

RSpec.describe RordanGramsay::Lint::Rubocop do
  ...
  describe '#rules' do
    it 'lists the rules broken' do
      File.write('metadata.rb', 'elsif')

      expect(obj.rules).not_to be_empty
      obj.rules.each do |rule|
        expect(rule).to be_a RordanGramsay::Rubocop::Rule
      end
    end
  end
end
