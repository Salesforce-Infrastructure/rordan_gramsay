...
RSpec.describe RordanGramsay::CLI do
  ...
  context "with 'init masterrakefile' commands" do
    it 'should create a Rakefile' do
      rakefile_contents = '# master rakefile'
      allow(cli).to receive(:master_rakefile_contents).and_return(rakefile_contents)
  ...
  context "with 'init masterrakefile --force' commands" do
    ...
    it 'should create a Rakefile' do
      ...
      expect(File.read('Rakefile')).to eq rakefile_contents
      expect(stdout).not_to match(/overwriting/i)
      expect(stdout).to match(/cookbook/i)
      expect(stdout).to match(/^created rakefile/i)
    end
